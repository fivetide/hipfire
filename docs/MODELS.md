...
TP×EP composition is explicitly refused (COMP-001): `params.tp` and `params.ep` cannot both exceed one (raw load-message degrees; only `--tp` is a first-class CLI flag — `hipfire serve --tp N` forwards `params.tp`). Reopening it requires a concrete deployment requirement and separate implementation/physical-validation task.
...
Qwen35-VL caveat: `daemon_load_plan` deliberately keeps Qwen35-VL on the
admission path (Planned `AXIS-004`) instead of the legacy PP exemption — a
`pp > 1` VL load is refused at admission (`[CAP-001]`, before any unload);
the legacy `pp > 1` HFQ exemption applies only to non-VL Qwen3.5. No PP+VL
support is claimed.
