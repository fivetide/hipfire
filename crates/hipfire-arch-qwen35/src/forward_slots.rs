...
use crate::qwen35::{
    moe_ffn_batched_admissible, moe_ffn_view_for_slots, mq6_batched_admit_enabled_from_env,
    prefill_moe_ffn_body_batched, q8_prefill_wmma_enabled, run_fused_gate_up_key,
    run_fused_qkv_key, run_fused_qkvza_key, run_plain_gemm_key, run_residual_gemm_key,
    DeltaNetLayerWeights, DeltaNetMoeLayerWeights, DeltaNetState, FullAttnLayerWeights,
    FullAttnMoeLayerWeights, LayerType, LayerWeights, MoeFfnView, PrefillBatchScratch,
    Qwen35Config, Qwen35MoeGroupPlans, Qwen35Scratch, Qwen35Weights, StateQuant,
};
...
fn require_batchable_moe_ffn(gpu: &Gpu, view: &MoeFfnView<'_>) -> HipResult<()> {
...
fn run_deltanet_moe_layer_slots(
    ...
    let ffn_view = moe_ffn_view_for_slots(&layer.ffn)?;
    require_batchable_moe_ffn(gpu, &ffn_view)?;
...
pub fn forward_batch_slots_opts(
    ...
    // the validated manifest authority, borrowed by layer per slot step. The
    // slot MoE layers are Legacy-only (admission-limited), so resolution runs
    // through the same model-owned cache as the reference chunk path.
