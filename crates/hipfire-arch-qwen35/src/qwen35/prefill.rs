...
use super::forward::map_bucket_err;
...
use super::forward::qwen35_prefill_phase_buckets;
use super::forward::MoeStepPhases;
...
use super::weights::MoeDtypeSnapshot;
...
use hipfire_dispatch::families::moe::MoePrefillResolution;
...
use hipfire_dispatch::pipeline::MoeActivationVariant;
use hipfire_dispatch::pipeline::MoeProj;
use hipfire_dispatch::pipeline::QwenDownMode;
...
use hipfire_runtime::llama::fused_silu_mul_rotate_mq_batched_with_awq;
    ...
    for (layer_idx, lw) in weights.layers.iter().enumerate() {
        match lw {
            ...
            LayerWeights::DeltaNetMoe(l) => {
                let ffn_mq3 = {
                    let view = weights.moe_ffn_view(layer_idx).map_err(|e| {
                        hip_bridge::HipError::new(
                            0,
                            &format!(
                                "forward_prefill_batch_single_chunk_captured: \
                                 moe_ffn_view(layer {layer_idx}) failed: {e}"
                            ),
                        )
                    })?;
                    moe_ffn_has_mq3_structural(&view) || moe_ffn_has_mq3_experts_uniform(&view)
                };
                if is_mq3_any(l.wqkv.gpu_dtype)
                    || is_mq3_any(l.wz.gpu_dtype)
                    || is_mq3_any(l.w_beta.gpu_dtype)
...
pub fn forward_prefill_batch_abortable(
    ...
    forward_prefill_batch_with_pbs_opts_inner(
        ...
        None, // mask_override: MTP probe is the only consumer; default callers don't override
        None, // max_layer: pflash uses this; non-pflash default is full stack
        true, // needs_last_token_logits: abortable trunk prefill keeps them
        None, // max_batch_cap
        abort,
    )
}
...
fn forward_prefill_batch_with_pbs_opts_inner(
            ...
            LayerWeights::DeltaNetMoe(l) => {
                let view = weights.moe_ffn_view(layer_idx).map_err(|e| {
                    hip_bridge::HipError::new(
                        0,
                        &format!(
                            "forward_prefill_batch: moe_ffn_view(layer {layer_idx}) failed: {e}"
                        ),
                    )
                })?;
                is_mq3_any(l.wqkv.gpu_dtype)
                    ...
                    || is_mq3_any(l.w_alpha.gpu_dtype)
                    || is_mq3_any(l.wo.gpu_dtype)
                    || moe_ffn_has_mq3_structural(&view)
                    || moe_ffn_has_unsupported_mq3_experts_uniform(&view, codebook_admit)
            }
            LayerWeights::FullAttnMoe(l) => {
                let view = weights.moe_ffn_view(layer_idx).map_err(|e| {
                    hip_bridge::HipError::new(
    ...
    let result = (|| -> HipResult<bool> {
        ...
        while chunk_start < n {
            ...
            // The chunk is the bounded unit of trunk work. Check before
            // committing any staging/ring writes and again after that GPU
            // work; an abort must never fall through as a successful
            // final-head result to the speculative caller.
            if abort() {
                return Ok(false);
            }
            if let Some(rb) = hidden_rb.as_mut() {
...
///
/// Bare-layer entry point: MoE storage is admitted through a `MoeFfnView`,
/// and only the `Legacy` variant can be resolved from a bare layer (the
/// Frozen projection lives in the `Qwen35Weights`-owned resident). Callers
/// that may hold Frozen MoE layers must use
/// [`qwen35_layer_batch_admissible_weights`].
pub fn qwen35_layer_batch_admissible(
    layer: &LayerWeights,
    config: &Qwen35Config,
    arch: &str,
) -> HipResult<()> {
    qwen35_layer_batch_admissible_inner(None, 0, layer, config, arch)
}

/// Weights-level per-layer batchability: identical checks to
/// [`qwen35_layer_batch_admissible`], but MoE storage is resolved through
/// the `Qwen35Weights` owner, so Frozen (resident-owned) MoE layers are
/// admitted from their resident projection instead of being refused. Dense
/// and Legacy-MoE layers behave byte-identically to the bare-layer entry
/// point.
pub(crate) fn qwen35_layer_batch_admissible_weights(
    weights: &Qwen35Weights,
    layer_idx: usize,
    config: &Qwen35Config,
    arch: &str,
) -> HipResult<()> {
    let layer = weights.layers.get(layer_idx).ok_or_else(|| {
        HipError::new(
            0,
            &format!("qwen35_layer_batch_admissible_weights: layer {layer_idx} out of range"),
        )
    })?;
    qwen35_layer_batch_admissible_inner(Some(weights), layer_idx, layer, config, arch)
}

/// Shared implementation. `weights` + `layer_idx` are `Some` only on the
/// weights-level path and let a `Frozen` MoE layer resolve its resident
/// projection; the bare-layer path passes `None` and refuses Frozen storage
/// with a diagnostic rather than silently skipping admission.
fn qwen35_layer_batch_admissible_inner(
    ...
    match layer {
        ...
        LayerWeights::DeltaNetMoe(l) => {
            ...
            // MoE geometry + admission go through a MoeFfnView: Legacy layers
            // resolve from their owned weights; Frozen layers resolve from the
            // weights-level resident (or are refused with a diagnostic on the
            // bare-layer path).
            let view = match (&l.ffn, weights) {
                (MoeFfnStorage::Legacy(ffn), _) => MoeFfnView::Legacy(ffn),
                (MoeFfnStorage::Frozen, Some(w)) => w.moe_ffn_view(layer_idx).map_err(|e| {
                    HipError::new(
                        0,
                        &format!(
                            "qwen35_layer_batch_admissible: moe_ffn_view(layer {layer_idx}): {e}"
                        ),
                    )
                })?,
                (MoeFfnStorage::Frozen, None) => {
                    return Err(HipError::new(
                        0,
                        "DeltaNetMoe Frozen MoE storage: bare-layer admission is Legacy-only — \
                         use qwen35_layer_batch_admissible_weights with the Qwen35Weights owner; \
                         refusing rather than silently skipping admission",
                    ));
                }
            };
            moe_ffn_layer_batch_admissible(&view, config, arch, "DeltaNetMoe")
        }
        LayerWeights::FullAttnMoe(l) => {
            if l.attn_norm.shape != vec![dim] {
            ...
            // MoE geometry + admission go through a MoeFfnView (see the
            // DeltaNetMoe arm above for the Legacy/Frozen resolution rules).
            let view = match (&l.ffn, weights) {
                (MoeFfnStorage::Legacy(ffn), _) => MoeFfnView::Legacy(ffn),
                (MoeFfnStorage::Frozen, Some(w)) => w.moe_ffn_view(layer_idx).map_err(|e| {
                    HipError::new(
                ...
                (MoeFfnStorage::Frozen, None) => {
                    return Err(HipError::new(
                        0,
                        "FullAttnMoe Frozen MoE storage: bare-layer admission is Legacy-only — \
                         use qwen35_layer_batch_admissible_weights with the Qwen35Weights owner; \
                         refusing rather than silently skipping admission",
                    ));
                }
            };
            moe_ffn_layer_batch_admissible(&view, config, arch, "FullAttnMoe")
        }
    }
}

/// View-based MoE geometry + admission tail for MoE layers, shared by the
/// bare-layer entry point's Legacy arm and the weights-level entry point's
/// resident-resolved arm so the two cannot drift. `label` prefixes the
/// error strings (`"DeltaNetMoe"` / `"FullAttnMoe"`), preserving the
/// original bare-layer diagnostics verbatim.
fn moe_ffn_layer_batch_admissible(
    view: &MoeFfnView<'_>,
    config: &Qwen35Config,
    arch: &str,
    label: &str,
    ...
    // MoE geometry.
    if view.router_m() != config.num_experts || view.router_k() != dim {
        return Err(HipError::new(0, &format!("{label} router shape mismatch")));
    }
    if view.shared_gate_m() != smi || view.shared_gate_k() != dim {
        return Err(HipError::new(
            0,
            &format!("{label} shared gate shape mismatch"),
        ));
    }
    if view.shared_up_m() != smi || view.shared_up_k() != dim {
        return Err(HipError::new(
            0,
            &format!("{label} shared up shape mismatch"),
        ));
    }
    if view.shared_down_m() != dim || view.shared_down_k() != smi {
        return Err(HipError::new(
            0,
            &format!("{label} shared down shape mismatch"),
        ));
    }
    if view.shared_expert_gate_m() != 1 || view.shared_expert_gate_k() != dim {
        return Err(HipError::new(
            0,
            &format!("{label} shared_expert_gate shape mismatch"),
        ));
    }
    // TopK gate is environment-sensitive.
    if !moe_prefill_topk_shape_supported(config.num_experts_per_tok, config.num_experts) {
        return Err(HipError::new(0, &format!("{label} topk shape unsupported")));
    }
    ...
    if !moe_ffn_batched_admissible(view, admit_mq6, arch) {
        return Err(HipError::new(
            0,
            &format!("{label} moe_ffn not batch-admissible"),
        ));
    }
    Ok(())
}
...
impl MoePrefillDtypes {
    ...
    /// View-based builder. Legacy defers to the exact `from_ffn` path
    /// (including the EP global-expert table when present); Frozen derives
    /// every field from the projection metadata accessors. Returns `None`
    /// when the layer has no routed experts (mirrors `from_ffn`'s
    /// empty-experts refusal).
    fn from_view(view: &MoeFfnView<'_>) -> Option<Self> {
        match view {
            MoeFfnView::Legacy(ffn) => Self::from_ffn(ffn),
            MoeFfnView::Frozen(_) => {
                if view.expert_count() == 0 {
                    return None;
                }
                Some(Self {
                    router: view.router_dtype(),
                    shared_expert_scalar_gate: view.shared_expert_gate_dtype(),
                    shared_expert_gate: view.shared_gate_dtype(),
                    shared_expert_up: view.shared_up_dtype(),
                    shared_expert_down: view.shared_down_dtype(),
                    expert_gate_up: view.first_expert_gate_up_dtype(),
                    expert_down: view.first_expert_down_dtype(),
                    expert_gate_up_uniform: view
                        .all_experts_gate_up_dtype(view.first_expert_gate_up_dtype()),
                    expert_down_uniform: view
                        .all_experts_down_dtype(view.first_expert_down_dtype()),
                    routed_mixed_merged: view.expert_dtype_tags_present(),
                })
            }
        }
    }

    /// Metadata-only builder from a `MoeDtypeSnapshot` (store-side Frozen
    /// admission seam). `None` when the snapshot has no routed experts —
    /// mirrors `from_ffn`'s empty-experts refusal.
    pub(crate) fn from_snapshot(snapshot: &MoeDtypeSnapshot) -> Option<Self> {
        if snapshot.expert_count == 0 {
            return None;
        }
        Some(Self {
            router: snapshot.router,
            shared_expert_scalar_gate: snapshot.shared_expert_scalar_gate,
            shared_expert_gate: snapshot.shared_gate,
            shared_expert_up: snapshot.shared_up,
            shared_expert_down: snapshot.shared_down,
            expert_gate_up: snapshot.expert_gate_up,
            expert_down: snapshot.expert_down,
            expert_gate_up_uniform: snapshot.expert_gate_up_uniform,
            expert_down_uniform: snapshot.expert_down_uniform,
            routed_mixed_merged: snapshot.expert_dtype_tags_present,
        })
    }
}
...
pub fn prefill_batch_pbs_eligible(
    ...
    let all_layers_ok = weights.layers.iter().enumerate().all(|(li, lw)| {
        if matches!(
            lw,
        ...
        // Weights-level admission resolves Frozen MoE storage through the
        // resident; the bare-layer entry point would refuse it.
        qwen35_layer_batch_admissible_weights(weights, li, config, arch).is_ok()
    });
...
// Env/arch-gated batched-prefill admission shared by the view-based and
// snapshot-based entry points below — the single source of truth so the
// Legacy and Frozen admission lattices cannot drift.
fn moe_ffn_batched_admissible_env_gated(
    dtypes: &MoePrefillDtypes,
    admit_mq6: bool,
    arch: &str,
) -> bool {
    ...
    static PARO_ADMIT: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
    ...
    {
        // Runtime-closure OnceLock: the note captures `dtypes` + `arch` from
        // the enclosing scope, which a const `LazyLock` initializer cannot.
...
// this file's own `prefill_batch_pbs_eligible` precondition check. The
// parameter is now a `MoeFfnView` (Legacy or Frozen) — forward_slots
// resolves its (Legacy-only) storage through `moe_ffn_view_for_slots`.
pub(crate) fn moe_ffn_batched_admissible(
    view: &MoeFfnView<'_>,
    admit_mq6: bool,
    arch: &str,
) -> bool {
    let Some(dtypes) = MoePrefillDtypes::from_view(view) else {
        return false;
    };
    moe_ffn_batched_admissible_env_gated(&dtypes, admit_mq6, arch)
}

/// Frozen-store admission seam: `MoeDtypeSnapshot` → the same env-gated
/// lattice as `moe_ffn_batched_admissible`, so `MoeDtypeSnapshot::batched_admissible`
/// (weights.rs) and `MoeFfnMetaView::batched_admissible` (store.rs) can admit
/// Frozen layers without constructing tensor bindings.
pub(crate) fn snapshot_batched_admissible(
    snapshot: &MoeDtypeSnapshot,
    admit_mq6: bool,
    arch: &str,
) -> bool {
    let Some(dtypes) = MoePrefillDtypes::from_snapshot(snapshot) else {
        return false;
    };
    moe_ffn_batched_admissible_env_gated(&dtypes, admit_mq6, arch)
}

/// Checked Legacy-only view adapter for the slot path (forward_slots.rs).
///
/// The MoE slots port is admission-limited to `MoeFfnStorage::Legacy` (its
/// own `require_batchable_moe_ffn` gate + the EP seal path guarantee it), so
/// a `Frozen` storage marker here is a programming error, not a fallback
/// case: return a diagnostic `Err` rather than fabricating weights or
/// unwrapping Frozen as Legacy. Generic paths should prefer
/// `Qwen35Weights::moe_ffn_view`, which resolves the resident projection.
pub(crate) fn moe_ffn_view_for_slots(ffn: &MoeFfnStorage) -> Result<MoeFfnView<'_>, HipError> {
    match ffn {
        MoeFfnStorage::Legacy(ffn) => Ok(MoeFfnView::Legacy(ffn)),
        MoeFfnStorage::Frozen => Err(HipError::new(
            0,
            "moe_ffn_view_for_slots: Frozen MoE storage on the slot path — \
             forward_slots is admission-limited to Legacy MoE storage; \
             refusing rather than fabricating weights",
        )),
    }
}
...
/// (Path 0/1/2; qwen-local port of the pre-merge dispatch
/// `build_moe_prefill_steps`). The model owns RMSNorm, routing (top-k), and
/// the shared expert — this builder emits only the routed block, in the
/// exact legacy `run_moe_prefill` launch order.
pub(crate) fn build_qwen_prefill_steps<'a>(
    p: &'a hipfire_dispatch::families::moe::MoePrefillParams<'a>,
...
/// [`crate::qwen35::forward::decode_expert_refs`]; dims come from the prefill
/// parameter set). O(1) — pointer tables + dims only.
pub(crate) fn prefill_expert_refs<'a>(
    p: &'a hipfire_dispatch::families::moe::MoePrefillParams<'a>,
...
/// partial). STEP-002 Task 9: the production batched routed tail builds the
/// dispatch builder's Step program and routes it through the runtime
/// lowerer under the Single policy + the MODEL-OWNED manifest group.
/// Path 0/1 (indexed) and Path 2 (grouped) both lower here with exact typed
/// membership; the sealed executor is the only prefill backend (no direct
/// `run_prefill` call remains for the migrated MoE prefill). The model owns
/// RMSNorm, routing, and the shared expert — this tail is the routed block
/// only.
///
/// `moe_dtypes` is the SAME EP-global-aware snapshot the body computed for
/// admission + dispatch (Legacy defers to the EP-global table when present),
/// so the lowered program resolves exactly like the direct executor it
/// replaces.
#[expect(
...
fn prefill_moe_ffn_routed_tail(
    ...
    let egu_ptrs = view.expert_gate_up_ptrs_tensor().map_err(moe_bind_err)?;
    let edn_ptrs = view.expert_down_ptrs_tensor().map_err(moe_bind_err)?;
    let edn_awq = view.expert_down_awq_ptrs_tensor().map_err(moe_bind_err)?;
    let edtags = view.expert_dtype_tags_tensor().map_err(moe_bind_err)?;
    ...
    let (gu_experts, dn_experts) = prefill_expert_refs(&moe_prefill_params);
    let phases = build_qwen_prefill_steps(&moe_prefill_params, &res, &gu_experts, &dn_experts)
        .map_err(HipError::from)?;
    ...
    let group = plans.by_layer(match view {
        MoeFfnView::Legacy(ffn) => ffn.layer_idx as usize,
        MoeFfnView::Frozen(b) => b.descriptors().layer_idx as usize,
    });
...
pub(crate) fn prefill_moe_ffn_body_batched(
    ...
    // forward through the validated manifest authority, borrowed by layer.
    plans: &Qwen35MoeGroupPlans,
) -> HipResult<()> {
    let dim = config.dim;
    let mi = config.moe_intermediate_size;
    ...
    // Split-Qwen storage: every weight below resolves through the view. Legacy
    // binds the owned `MoeFfnWeights` tensors; Frozen binds the resident store
    // cells. AWQ sidecars ride on the `WeightRef`s (`awq_scale`), so the
    // AWQ-aware rotate/silu dispatches stay byte-identical for Legacy and gain
    // the store-side companions for Frozen. Bind failures propagate as
    // diagnostics — never a panic on a production Frozen route.
    let router_w = view.router_ref().map_err(moe_bind_err)?;
    let shared_scalar_w = view.shared_expert_gate_ref().map_err(moe_bind_err)?;
    let shared_gate_w = view.shared_gate_ref().map_err(moe_bind_err)?;
    let shared_up_w = view.shared_up_ref().map_err(moe_bind_err)?;
    let shared_down_w = view.shared_down_ref().map_err(moe_bind_err)?;
    ...
    let paro_mode = matches!(shared_gate_w.dtype, DType::ParoQ4G128);
    if !paro_mode {
        // AWQ-aware rotate resolved from the view's shared-gate sidecar — the
        // same dispatch `rotate_x_mq_batched_for` performs on the
        // WeightTensor's `awq_scale`, byte-identical for Legacy.
        if let Some(awq) = shared_gate_w.awq_scale {
    ...
    // are registered ArchPredicate::Always, so run_key never rejects.
    {
        use hipfire_dispatch::families::gemm::GemmParams;
        let ctx = DispatchCtx::new(gpu);
        let (key, x_in): (hipfire_dispatch::types::KernelKey, &GpuTensor) = match router_w.dtype {
            DType::Q8_0 => (
        ...
        let w = WeightRef {
            buf: router_w.buf,
            dtype: router_w.dtype,
            m: router_w.m,
            k: router_w.k,
            row_stride: router_w.k,
            rotation: None,
            awq_scale: None,
        };
        let params = GemmParams {
    ...
    dump_hidden_localize(gpu, router_logits, n, 0, router_w.m, 0, "router_b");
    ...
    // gpu.gemm_* method, byte-for-byte.
    {
        ...
        let (key, x_in): (KernelKey, &GpuTensor) = match shared_scalar_w.dtype {
    ...
    match shared_gate_w.dtype {
        ...
        DType::MQ4G256 | DType::MQ4G256V2 => run_fused_gate_up_key(
            gpu,
            crate::forward_slots::fused_gate_up_key_for(shared_gate_w.dtype),
            shared_gate_w.buf,
            shared_up_w.buf,
            &pbs.x_rot_batch,
            shared_gate,
        ...
        DType::ParoQ4G128 => {
            // PARO carries per-weight Givens tables only on Legacy storage
            // (Frozen has no PARO route — `paro_shared_present` is false and
            // the store validation rejects ParoQ4G128 projections). Refuse
            // with a diagnostic instead of panicking on a production route.
            let (paro_gate, paro_up) = match view {
                ...
                MoeFfnView::Frozen(_) => {
                    return Err(HipError::new(
                        0,
                        "prefill_moe_ffn_body_batched: ParoQ4G128 shared gate/up on Frozen \
                         storage — Frozen has no PARO Givens route; refusing rather than \
                         mis-executing",
                    ));
                }
            };
        ...
        DType::MFP4G32E8 => {
            let gate_m = shared_gate_w.m;
            let gate_k = shared_gate_w.k;
            let up_m = shared_up_w.m;
            let up_k = shared_up_w.k;
            ...
            gpu.dequantize_mfp4g32_e8_to_f16(
                &shared_gate_w.buf.buf,
                &gate_f16.buf,
                gate_m,
                gate_k,
            )?;
            let up_f16 = gpu.alloc_tensor(&[up_m * up_k], DType::F16)?;
            gpu.dequantize_mfp4g32_e8_to_f16(&shared_up_w.buf.buf, &up_f16.buf, up_m, up_k)?;
    ...
    if paro_mode {
        let paro_down = match view {
            ...
            MoeFfnView::Frozen(_) => {
                return Err(HipError::new(
                    0,
                    "prefill_moe_ffn_body_batched: ParoQ4G128 shared down on Frozen storage — \
                     refusing rather than mis-executing",
                ));
            }
        };
    ...
    } else if matches!(shared_down_w.dtype, DType::Q8_0) {
        ...
        // The projected view carries the same optional sidecar as a resident
        // WeightTensor, so both Legacy and Frozen use the shared runtime
        // dispatch without architecture-local raw kernel calls.
        fused_silu_mul_rotate_mq_batched_with_awq(
            gpu,
            shared_down_w.awq_scale,
            shared_gate,
            shared_up,
            shared_rot,
            smi,
    ...
    match shared_down_w.dtype {
        ...
        DType::MFP4G32E8 => {
            let down_m = shared_down_w.m;
            let down_k = shared_down_w.k;
            let down_f16 = gpu.alloc_tensor(&[down_m * down_k], DType::F16)?;
            gpu.dequantize_mfp4g32_e8_to_f16(
                &shared_down_w.buf.buf,
                &down_f16.buf,
                down_m,
                down_k,
            )?;
    ...
    // The production batched routed tail builds the dispatch builder's Step
    // program and routes it through the runtime lowerer under the Single
    // policy + the MODEL-OWNED manifest group. Path 0/1 (indexed) and
    // Path 2 (grouped) both lower with exact typed membership; the sealed
    // executor is the only prefill backend for the routed block. The model
    // owns RMSNorm, routing, and the shared expert (steps 1-5 above stay
    // direct).
    ...
    // path. This prefill site always has ≥1 expert. Legacy defers to the
    // EP-global-aware free fn (the local shard slice may look uniform on each
    // rank while the global model is graded); Frozen derives the tiers from
    // the resident projection.
    let (per_expert_gate_up, per_expert_down) = match view {
        MoeFfnView::Legacy(ffn) => per_expert_tier_tables(ffn),
        MoeFfnView::Frozen(_) => view.per_expert_tier_tables(),
    };
    // Routed dtype fields keep the EP-global-table precedence for Legacy (the
    // rank-local `experts` slice may look uniform while the global file is
    // graded mixed — see `MoePrefillDtypes::from_ffn`); Frozen has no global
    // table and derives everything from the projection.
    let (experts_all_gate_up_mq4, routed_gate_up, routed_down) = match view {
        ...
        MoeFfnView::Frozen(_) => (
            view.experts_all_gate_up_mq4(),
            view.first_expert_gate_up_dtype(),
            view.first_expert_down_dtype(),
        ),
    };
    let moe_dtypes = hipfire_dispatch::families::moe::MoeDtypes {
        router: router_w.dtype,
        shared_gate: shared_scalar_w.dtype,
        shared_expert_gate: shared_gate_w.dtype,
        shared_expert_up: shared_up_w.dtype,
        shared_expert_down: shared_down_w.dtype,
        ...
        // AWQ capability fields (mainline names): actual routed/shared sidecar
        // semantics from the view. `gate_side_has_awq` disables the fused gate
        // path in decode resolution; `routed_down_has_awq` suppresses prefill
        // Path 2 (grouped-GEMM) — both must be carried honestly, never dropped.
...
/// Map a `MoeFfnView` tensor-bind failure into the prefill error domain with
/// the body's context. Frozen bind failures propagate as diagnostics — never
/// a panic on a production Frozen route.
fn moe_bind_err(e: crate::store::Qwen35MoeBindError) -> HipError {
    HipError::new(0, &format!("prefill_moe_ffn_body_batched: {e}"))
}
...
fn batch_chunk_delta_net_moe(
    ...
    // batched indexed down kernel's atomicAdd path. Storage is
    // resolved through the weights-owned view (Legacy or Frozen).
    let ffn_view = weights.moe_ffn_view(layer_idx).map_err(|e| {
        HipError::new(
            0,
            &format!("batch_chunk_delta_net_moe: moe_ffn_view(layer {layer_idx}): {e}"),
        )
    })?;
...
fn batch_chunk_full_attn_moe(
    ...
    // Batched MoE FFN. Storage is resolved through the weights-owned view
    // (Legacy or Frozen).
    let ffn_view = weights.moe_ffn_view(layer_idx).map_err(|e| {
        HipError::new(
            0,
            &format!("batch_chunk_full_attn_moe: moe_ffn_view(layer {layer_idx}): {e}"),
        )
    })?;
    prefill_moe_ffn_body_batched(
        gpu,
...
pub(crate) fn forward_batch_chunk_impl(
    ...
    let fa_batched_ok =
        (kv_cache.quant_q8 || kv_cache.quant_asym4 || kv_cache.quant_asym3 || kv_cache.quant_asym2)
            && weights.layers.iter().enumerate().all(|(li, lw)| match lw {
                LayerWeights::FullAttn(_) | LayerWeights::FullAttnMoe(_) => {
                    qwen35_layer_batch_admissible_weights(weights, li, config, fa_arch).is_ok()
                }
                _ => true,
            });
...
mod tests {
    ...
    // ── STEP-002 Task 9: prefill routed tail → sealed Single lowering ─────
    // CPU-pure: synthetic tensors with honest byte capacities (never touched
    // by HIP); the lowerer's capacity checks see real sizes.

    use hipfire_dispatch::families::moe::{ExpertExecutionPlan, MoePrefillResolution, RouterPlan};
    ...
    /// mi=128, n_exp=8) with honest synthetic capacities.
    ...
    /// Params with CAPACITY-owned batch buffers: `x_batch`/`routed_out` sized
    /// for `capacity` tokens while the active chunk is `n_active` (the
    /// production PBS capacity-vs-active split).
    ...
    /// The production prefill lowering seam (`prefill_moe_ffn_routed_tail`'s
    /// path): build → bucket → lower through the single-policy model-owned
    /// group. Returns the lowerer's typed error on failure.
    fn lower_prefill_params(
        ...
        let phases =
            build_qwen_prefill_steps(params, &res, &gu, &dn).expect("builder accepts the params");
    ...
    fn qwen35_prefill_path1_indexed_reaches_runtime_lowering() {
        ...
        let (gu, dn) = prefill_expert_refs(&params);
        let buckets = {
            let phases = build_qwen_prefill_steps(&params, &res, &gu, &dn).unwrap();
            qwen35_prefill_phase_buckets(phases).unwrap()
        };
        ...
        lower_prefill_params(&params, false, false).expect("indexed prefill must lower");
    }
    ...
    fn qwen35_prefill_path2_grouped_reaches_runtime_lowering() {
        ...
        // Path 2: [scatter] → [grouped gate_up, gate-up deinterleave] →
        // activation → grouped down → inverse-permuting combine.
        ...
        lower_prefill_params(&params, true, false).expect("grouped prefill must lower");
    }
    ...
    fn qwen35_prefill_path0_atomic_reaches_runtime_lowering() {
        ...
        // Path 0: gate_up → activation → self-combining down (no combine).
        assert_eq!(buckets.router.len(), 0);
        assert_eq!(buckets.gate_up.len(), 1);
        assert_eq!(buckets.activation.len(), 1);
        assert_eq!(buckets.down.len(), 1);
        ...
        assert!(matches!(buckets.gate_up[0], Step::MoeGateUpIndexed { .. }));
        assert!(matches!(buckets.down[0], Step::MoeDownIndexed { .. }));
        lower_prefill_params(&params, false, true).expect("path-0 prefill must lower");
    }

    #[test]
    fn qwen35_prefill_grouped_steps_rejected_under_indexed_claim() {
        // A grouped Path-2 program claimed as IndexedQuantized is REFUSED
        // with the shared ExecutionIdentityMismatch — membership alone must
        // never admit a mislabeled grouped schedule.
        let cfg = task9_moe_config();
        let params = prefill_params(8);
        let res = MoePrefillResolution {
            use_path2: true,
    ...
    fn qwen35_prefill_capacity_owner_view_repairs_combine_shape() {
        ...
        // 1. The unviewed capacity owner still REFUSES (protocol strict).
        ...
        // 2. The production view: pointer identity, dtype, full byte
        //    capacity, exact `[n, dim]` shape.
        ...
        //    Path 0/1/2 all lower successfully.
