...
use hipfire_runtime::hfq::load_awq_scale;
...
fn attach_awq_scale_candidates(
    ...
    for name in names {
        match load_awq_scale(hfq, gpu, name, k) {
            ...
            Ok(None) => continue,
            Err(error) => {
                // The primary weight buffer is uploaded but not yet in the
                // caller's staging set. Free it before propagating the sidecar
                // error so the outer staging transaction can reclaim earlier
                // allocations without leaking this weight.
                wt.free_all(gpu);
                return Err(error);
            }
        }
...
/// Load one quantized weight tensor (candidate-resolved) with the Phase A
/// Stage A AWQ sidecar attached. `gpu` is `&mut` because the sidecar upload
/// (see [`attach_awq_scale_candidates`]) may allocate.
...
impl WeightSource for HfqSource<'_> {
    ...
    fn read_output(
        ...
        let (output, aliases) = resolve_lm_head(
            gpu,
            has_separate,
            can_alias,
            embd,
        ...
        let output = attach_lm_head_awq_sidecar(self.hfq, gpu, output, c.dim)?;
        Ok((output, aliases))
    }
...
fn try_load_packed_mq4_experts(
    ...
    for (slot, spec) in specs.iter().enumerate() {
        ...
        gate_up.awq_scale = match load_awq_scale(hfq, gpu, &spec.gate_up_name, dim) {
            Ok(scale) => scale,
            // Per-expert buffers are views into the pool owners; free the pools
            // (the real allocations) before propagating so the caller's
            // transactional staging can reclaim prior layers without leaking
            // this expert pool.
            Err(error) => {
                let _ = gpu.free_tensor(gate_up_owner);
                let _ = gpu.free_tensor(down_owner);
                return Err(error);
            }
        };
        ...
        down.awq_scale = match load_awq_scale(hfq, gpu, &spec.down_name, mi) {
            Ok(scale) => scale,
            Err(error) => {
                let _ = gpu.free_tensor(gate_up_owner);
                let _ = gpu.free_tensor(down_owner);
