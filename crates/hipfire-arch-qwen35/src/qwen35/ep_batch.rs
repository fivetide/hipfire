...
use super::weights::reject_frozen_multi;
...
fn layer_moe_ffn(layer: &LayerWeights) -> Option<&MoeFfnStorage> {
    match layer {
...
/// Frozen storage, REAP/paged/AWQ/GL, Q8/EF, capacities, LDS. Returns attested receipt on success.
pub fn validate_ep_batch_compatibility(
    ...
    for (idx, w) in weights_per_rank.iter().enumerate() {
        let prov = w.ep_shard().ok_or_else(|| HipError::new(0, &format!("EP batch: rank {idx} missing EP shard provenance (only load_weights_ep_rank may attach)")))?;
        ...
        for layer in &w.layers {
            ...
            match layer {
                ...
                LayerWeights::DeltaNetMoe(l) => {
                    ...
                    check_weight(&l.wo, "wo")?;
                    let ffn = match &l.ffn {
                        MoeFfnStorage::Legacy(ffn) => ffn,
                        MoeFfnStorage::Frozen => {
                            return Err(HipError::new(
                                0,
                                &format!(
                                    "EP batch: rank {idx} Frozen MoE storage not supported (EP batch requires Legacy load_weights_ep_rank shards)"
                                ),
                            ));
                        }
                    };
                    check_weight(&ffn.router, "router")?;
                    check_weight(&ffn.shared_expert.gate, "shared_gate")?;
                    check_weight(&ffn.shared_expert.up, "shared_up")?;
                    check_weight(&ffn.shared_expert.down, "shared_down")?;
                    check_weight(&ffn.shared_expert_gate, "shared_expert_gate")?;
                    ...
                    match (&ffn.expert_dtype_tags, is_mixed) {
                        (Some(tags), true) => {
                    ...
                    if ffn.expert_gate_up_ptrs.shape != vec![2 * config.num_experts] {
                        return Err(HipError::new(
                            0,
                            "EP batch: gate_up pointer table shape mismatch",
                        ));
                    }
                    if ffn.expert_down_ptrs.shape != vec![2 * config.num_experts] {
                        return Err(HipError::new(
                            0,
    ...
    for li in 0..config.n_layers {
        ...
        for (r_idx, w) in weights_per_rank.iter().enumerate() {
            ...
            if let Some(storage) = layer_moe_ffn(layer) {
                let global = match storage {
                    MoeFfnStorage::Legacy(ffn) => ffn.global_expert_dtypes.as_ref().ok_or_else(
                        || {
                            HipError::new(
                                0,
                                &format!(
                    ...
                    MoeFfnStorage::Frozen => {
                        return Err(HipError::new(
                            0,
                            &format!(
                                "EP batch: rank {r_idx} layer {li} Frozen MoE storage not supported"
                            ),
                        ));
                    }
                };
    ...
    if let Some(assign) = &ref_assign {
        ...
        for (idx, w) in weights_per_rank.iter().enumerate() {
            let prov = w.ep_shard().ok_or_else(|| {
            ...
            for (li, layer) in w.layers.iter().enumerate() {
                if let Some(storage) = layer_moe_ffn(layer) {
                    let n_local = match storage {
                        MoeFfnStorage::Legacy(ffn) => ffn.experts.len(),
                        MoeFfnStorage::Frozen => {
                            return Err(HipError::new(
                                0,
                                &format!(
                                    "EP batch: rank {idx} layer {li} Frozen MoE storage not supported"
                                ),
                            ));
                        }
                    };
                    if n_local != expected_owned {
                        return Err(HipError::new(0, &format!("EP batch: rank {idx} layer {li} loaded {n_local} experts but assignment expects {expected_owned}")));
                    }
                }
            }
        }
...
pub fn forward_ep(
    ...
    for w in weights_per_rank {
        reject_frozen_multi("forward_ep", w)?;
    }
    ...
    // STEP-002 Task 9: per-rank model-owned expert-group plans — resolved ONCE
    // per forward from each rank's weights owner (the OnceLock authority) and
    // borrowed by the SuperOp MoE bindings; never re-resolved per layer/token/
    // rank.
    let plans_per_rank: Vec<&Qwen35MoeGroupPlans> = weights_per_rank
        .iter()
        .map(|w| w.moe_group_plans(config).map_err(|e| HipError::new(0, &e)))
        .collect::<Result<_, _>>()?;
    ...
    for layer_idx in 0..config.n_layers {
        ...
        for (r, (((w, s), kv), dn)) in weights_per_rank
            .iter()
...
pub fn forward_prefill_batch_ep(
    ...
    for w in weights_per_rank {
        reject_frozen_multi("forward_prefill_batch_ep", w)?;
    }
    ...
    let ep_skip_ar = hipfire_config::developer_var("HIPFIRE_EP_SKIP_ALLREDUCE").is_ok(); // DIAGNOSTIC ONLY (wrong output)
                                                                                         ...
                                                                                         // Full-rank all-reduce group: the routed partials are rank-ordered
                                                                                         // (`partials[k]` lives on `devices[k]`), so the group is exactly 0..n_rank.
                                                                                         // Built once per prefill call, outside the per-layer loop.
    let group: Vec<usize> = (0..n_rank).collect();
    // Peer-direct all-reduce (bypass RCCL): the routed-partial sum goes through
    // `Gpus::all_reduce_sum_f32_peer` (direct P2P copy + local add), which is
    // ~1 ms vs RCCL's ~40 ms/call on hiptrx (gfx1201, PCIe). DEFAULT ON; opt
    // back to RCCL with HIPFIRE_EP_PEER_ALLREDUCE=0.
    let ep_peer_ar =
...

        {
            ...
            match (&weights.layers[layer_idx], config.layer_types[layer_idx]) {
                ...
                (LayerWeights::DeltaNetMoe(layer), LayerType::LinearAttention) => {
                    ...
                    let ffn = layer.ffn.as_legacy().expect(
                        "forward_scratch_layers_multi: Legacy-only via reject_frozen_multi",
                    );
