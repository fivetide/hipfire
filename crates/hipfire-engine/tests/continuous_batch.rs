...
use hipfire_engine::terminal::{
    ...
    batch_transfer_abort_to_singleton_and_clear, batch_transition_to_queued, AttemptKey,
...
#[test]
fn cohort_admission_key_follows_front_then_active_then_clears() {
    let _l = begin();
    let mut sched = ContinuousBatchScheduler::new(2, 4096);
    // No cohort, no pending: any cohort may join.
    assert!(sched.cohort_admission_key().is_none());
    let k1 = AttemptKey::new("admit-1", 1);
    let k2 = AttemptKey::new("admit-2", 2);
    let s_a = sampling(0.3, 1.0);
    let s_b = sampling(0.7, 1.0);
    batch_announce_terminal(&k1.id, k1.attempt_id);
    sched.enqueue(req_with_tokens(
        k1.clone(),
        s_a.clone(),
    ...
    // Unassigned front pending sets the effective admission cohort.
    assert_eq!(sched.cohort_admission_key(), Some(s_a.key()));
    let (_k1, t1) = sched.try_assign_one().unwrap();
    // Active cohort now governs admission.
    assert_eq!(sched.cohort_admission_key(), Some(s_a.key()));
    let pending =
        serde_json::json!({"type":"done","id":k1.id,"attempt_id":k1.attempt_id,"tokens":1});
    assert!(sched.mark_awaiting_commit(t1.lane, pending));
    batch_apply_terminal_control("commit", &k1.id, k1.attempt_id);
    assert!(sched.commit_lane(t1.lane, &k1));
    // Cohort cleared: next request sets its own cohort.
    assert!(sched.cohort_admission_key().is_none());
    batch_announce_terminal(&k2.id, k2.attempt_id);
    sched.enqueue(req_with_tokens(
        k2.clone(),
    ...
    assert_eq!(sched.cohort_admission_key(), Some(s_b.key()));
}

#[test]
fn cohort_mismatch_park_leaves_announcement_latched_and_readmittable() {
    let _l = begin();
    // The driver parks a cohort-mismatched request BEFORE enqueue; the
    // reader's announcement must survive so abort works while parked and
    // the daemon's re-admission can transition Announced -> Queued.
    let k1 = AttemptKey::new("park-1", 1);
    batch_announce_terminal(&k1.id, k1.attempt_id);
    assert!(batch_transition_to_queued(&k1.id, k1.attempt_id));
    // Abort latches while parked (registry untouched by the driver park).
    batch_apply_terminal_control("abort", &k1.id, k1.attempt_id);
    assert!(batch_check_abort(&k1.id, k1.attempt_id));
    batch_clear_terminal(&k1.id, k1.attempt_id);
    // A fresh re-admission (daemon, next wave) works from Announced.
    batch_announce_terminal(&k1.id, k1.attempt_id);
    assert!(batch_transition_to_queued(&k1.id, k1.attempt_id));
    batch_clear_terminal(&k1.id, k1.attempt_id);
}
...
#[test]
fn think_requeue_preserves_original_message_field_equivalence() {
    let _l = begin();
    let k = AttemptKey::new("think-req", 7);
    let original = serde_json::json!({
        "type": "generate",
        "id": k.id,
        "attempt_id": k.attempt_id,
        "prompt": "explain quaternions",
        "system": "you are terse",
        "messages": [{"role": "user", "content": "explain quaternions"}],
    ...
    let mut sched = ContinuousBatchScheduler::new(2, 4096);
    batch_announce_terminal(&k.id, k.attempt_id);
    sched.enqueue(BatchPendingRequest {
        original_msg: original.clone(),
        key: k.clone(),
        prompt: "explain quaternions".into(),
        prompt_tokens: vec![1, 2, 3],
        ...
        max_think_tokens: 512,
        max_tokens: 128,
        sampling: sampling(0.4, 1.05),
    });
    // The driver's requeue path: park the ORIGINAL message value, never a
    // reconstructed envelope — byte/field-equivalent by Value equality.
    let stored = sched.pending.get(&k).unwrap().original_msg.clone();
    assert_eq!(stored, original);
    let parked_msg = DaemonMsg::Regular(stored);
    match &parked_msg {
        DaemonMsg::Regular(v) => {
            assert_eq!(v, &original);
            assert_eq!(
                v.get("system").and_then(|x| x.as_str()),
                Some("you are terse")
            );
            assert!(v.get("messages").is_some());
            assert_eq!(
                v.get("assistant_prefix").and_then(|x| x.as_str()),
                Some("open_think")
            );
            assert_eq!(
                v.get("max_think_tokens").and_then(|x| x.as_u64()),
                Some(512)
            );
            assert_eq!(v.get("temperature").and_then(|x| x.as_f64()), Some(0.4));
        }
        DaemonMsg::ParseError(_) => panic!("expected Regular"),
    }
}

#[test]
fn daemon_inbox_parking_keeps_message_for_outer_loop() {
    let _l = begin();
    let (tx, rx) = std::sync::mpsc::channel::<DaemonMsg>();
    let mut inbox = DaemonInbox::new(rx);
    let msg = DaemonMsg::Regular(serde_json::json!({"type":"reset","attempt_id":1}));
    let msg_value = serde_json::json!({"type":"reset","attempt_id":1});
    // Driver parks: push to the back and skip re-processing via value check.
    inbox.push_back(msg.clone());
    let popped = inbox.try_recv().unwrap();
    assert!(matches!(&popped, DaemonMsg::Regular(v) if v == &msg_value));
    // Driver skip-check cycle: the popped parked message is pushed back
    // (value compare) and re-popped unchanged — never reprocessed, never
    // lost, and still served to the daemon's outer loop after the wave.
    inbox.push_back(popped);
    let again = inbox.try_recv().unwrap();
    assert!(matches!(&again, DaemonMsg::Regular(v) if v == &msg_value));
    // A fresh message from the channel is reachable behind it.
    tx.send(DaemonMsg::Regular(serde_json::json!({"type":"ping"})))
        .unwrap();
    let third = inbox.try_recv().unwrap();
    assert!(
        matches!(third, DaemonMsg::Regular(v) if v.get("type").and_then(|x| x.as_str()) == Some("ping"))
    );
}

#[test]
fn per_slot_commit_and_abort_are_independent() {
    let _l = begin();
    let mut sched = ContinuousBatchScheduler::new(2, 4096);
    let k1 = AttemptKey::new("slot-a", 1);
    let k2 = AttemptKey::new("slot-b", 2);
    let s = sampling(0.3, 1.0);
    batch_announce_terminal(&k1.id, k1.attempt_id);
    batch_announce_terminal(&k2.id, k2.attempt_id);
    sched.enqueue(req(k1.clone(), s.clone()));
    sched.enqueue(req(k2.clone(), s.clone()));
    let (_a, t1) = sched.try_assign_one().unwrap();
    let (_b, t2) = sched.try_assign_one().unwrap();
    assert_ne!(t1.lane, t2.lane);
    // Abort slot A only: slot B must keep its key/state untouched.
    batch_apply_terminal_control("abort", &k1.id, k1.attempt_id);
    assert!(sched.abort_lane(t1.lane, &k1));
    assert!(sched.lanes[t1.lane].is_empty());
    assert_eq!(sched.find_lane_by_key(&k2), Some(t2.lane));
    assert_eq!(sched.find_lane_by_key(&k1), None);
    assert!(!batch_check_abort(&k2.id, k2.attempt_id));
    // Commit slot B independently; A's cleared terminal must not interfere.
    let pending =
        serde_json::json!({"type":"done","id":k2.id,"attempt_id":k2.attempt_id,"tokens":2});
    assert!(sched.mark_awaiting_commit(t2.lane, pending));
    batch_apply_terminal_control("commit", &k2.id, k2.attempt_id);
    assert!(sched.commit_lane(t2.lane, &k2));
    assert!(sched.lanes[t2.lane].is_empty());
    assert_eq!(sched.active_count(), 0);
}

#[test]
#[should_panic(expected = "64 lane hard cap")]
fn scheduler_refuses_over_64_lanes_before_construction() {
    let _l = begin();
    // u64 lane masks cannot represent >64 lanes; the constructor must
    // refuse loudly (staging fail-closes the same bound pre-GPU).
    let _ = ContinuousBatchScheduler::new(65, 4096);
}
