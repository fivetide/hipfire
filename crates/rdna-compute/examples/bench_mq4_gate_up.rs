...
fn bench(m: usize, k: usize, n: usize, runs: usize) {
    ...
    for _ in 0..3 {
        gpu.gemm_gate_up_mq4g256_lloyd_wmma(&d_ag, &d_au, &d_x, &d_yg, &d_yu, gm, um, k, n).unwrap();
    }
...
fn main() {
    let shapes = [(27648, 8192usize, 64usize), (6912, 8192, 64), (27648, 8192, 16)];
    for &(m, k, n) in &shapes {
        bench(m, k, n, 10);
    }
}
