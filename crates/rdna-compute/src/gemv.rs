...
impl Gpu {
    ...
    /// MQ2L int64 expanded MoE down-GEMV (K4). Writes raw i64 bits (signed
    /// int64 cells) to expert_outputs_i64; convert with `moe_i64_residual_to_f32`
    /// element-wise. topk_weights is NOT consumed here.
    pub fn moe_down_mq2g256_lloyd_expanded_i64(
        &mut self,
        expert_ptrs: &GpuTensor,        // [n_exp]
        topk_indices: &GpuTensor,       // [N × K_TOP]
        rot_batch: &GpuTensor,          // [N × K_TOP × K]
        expert_outputs_i64: &GpuTensor, // [N × K_TOP × M] raw i64 cells
        m: usize,
        k: usize,
        k_top: usize,
        batch_size: usize,
        ...
        self.bind_thread()?;
        let logical_name = "gemv_mq2g256_lloyd_moe_down_expanded_i64_k4";
        let source = kernels::GEMV_MQ2G256_LLOYD_MOE_DOWN_EXPANDED_K4_SRC;
        let symbol = "gemv_mq2g256_lloyd_moe_down_expanded_i64_k4";
        self.ensure_kernel(logical_name, source, symbol)?;
        let pp = expert_ptrs.buf.as_ptr();
        let ip = topk_indices.buf.as_ptr();
        let xp = rot_batch.buf.as_ptr();
        let yp = expert_outputs_i64.buf.as_ptr();
        let m_val = m as i32;
        let k_val = k as i32;
        let kt_val = k_top as i32;
        let mut params: Vec<*mut c_void> = vec![
        ...
        let mq2_weight_bytes = m * (k / 256) * 72;
        let bytes = batch_size * (k_top as usize) * (mq2_weight_bytes + k * 4 + m * 8);
        let timer = crate::profile::begin_timer(&self.hip, "gemv", logical_name, bytes);
        let result = self.launch_maybe_blob(
            symbol,
            [m as u32, k_top as u32, batch_size as u32],
