...
/// Dense-LFM bundle gate for batch admission: route must be LfmAr, the
/// batch state must be staged, the config must be dense (MoE LFM is never
/// batch-eligible), and the weight formats must admit the batched kernels.
/// Pure — extracted so the dense-LFM admission contract is pinned by tests
/// without a GPU. There is deliberately no unconditional false: a bundle
/// that passes every gate IS batch-eligible (B1).
pub fn lfm_bundle_batch_gate(
    route: GenerationRoute,
    has_batch_state: bool,
    config_is_dense: bool,
    weight_formats_ok: bool,
) -> bool {
    route == GenerationRoute::LfmAr && has_batch_state && config_is_dense && weight_formats_ok
}
...
pub fn is_batch_request_eligible(
    ...
    if caps.supports_continuous_batch {
        if let Some(bundle) = m.state.as_ref().and_then(|s| {
            ...
            if !lfm_bundle_batch_gate(
                route,
                bundle.lfm2_decode_batch.is_some(),
                bundle.config.is_dense(),
                lfm2moe::batch_weight_formats_supported(&bundle.weights).is_ok(),
            ) {
                return false;
            }
        }
...
pub fn drive_qwen_continuous_batch(
    ...
    if batch_size > ContinuousBatchScheduler::MAX_BATCH_LANES {
        // Refuse before any GPU mutation: decode batch states address lanes
        // with u64 masks and cannot represent more than 64 lanes. Staging
        // fail-closes this earlier; this guard keeps a misconstructed
        // scheduler from poisoning the batch on first use.
        return Err(BatchDriveError::Gpu(format!(
            "continuous batch size {batch_size} exceeds the {} lane hard cap (u64 lane masks)",
            ContinuousBatchScheduler::MAX_BATCH_LANES
        )));
    }
    ...
    loop {
        ...
        loop {
            ...
            match dm {
                ...
                DaemonMsg::Regular(json) => {
                    ...
                    if t == "generate" {
                        if parked.contains(&json) {
                            // Already parked: keep it at the back; the daemon
                            // serves it between waves after this drive returns.
                            inbox.push_back(DaemonMsg::Regular(json));
                            break;
                        }
                        if !parked.is_empty() {
                            // A barrier is parked: bound this wave — do not
                            // admit new work until the daemon services it.
                            parked.push(json.clone());
                            inbox.push_back(DaemonMsg::Regular(json));
                            break;
                        }
                        ...
                        if let Some(cohort) = sched.cohort_admission_key() {
                            if cohort != sampling.key() {
                                // Cohort-mismatched request: park the ORIGINAL
                                // message before announcing/enqueueing it. The
                                // reader's announcement stays latched so abort
                                // works while parked; the daemon re-admits the
                                // request between waves once the cohort drains
                                // (exactly one gen_start, no semantic loss).
                                parked.push(json.clone());
                                inbox.push_back(DaemonMsg::Regular(json));
                                break;
                            }
        ...
        if !parked.is_empty() && sched.active_count() == 0 && sched.inbox.is_empty() {
            // A barrier is parked: return so the daemon's sequential loop
            // serves it between batch waves (bounded barrier service).
            return Ok(());
        }
        while let Some((key, ticket)) = sched.try_assign_one() {
            ...
            if started_in_think {
                ...
                // requeue the ORIGINAL message (byte/field-equivalent) for
                // outer sequential handling — never reconstruct it field-wise.
                ...
                let original = pending_req.original_msg.clone();
                parked.push(original.clone());
                inbox.push_back(DaemonMsg::Regular(original));
                break;
            }
            if prompt_tokens.is_empty() || prompt_tokens.len() >= sched.lane_capacity {
                // Defensive re-check at assignment: the daemon's admission
                // bounds by m.max_seq, which can exceed the clamped
                // lane_capacity. Fail this lane only — never fail_all — so
                // peers keep decoding (mirrors the LFM assignment re-check).
        ...
        let awaiting: Vec<usize> = sched
            ...
            .collect();
        if running.is_empty() && awaiting.is_empty() && sched.inbox.is_empty() {
            break;
        }
        if running.is_empty() {
                    ...
                    if t == "generate" {
                        ...
                        if batch_check_abort(&id, attempt_id) {
                            ...
                            emit_gen_start(
                                ...
                                &id,
                                false,
                                crate::common::gen_start_contract_version_for_arch(model.arch_id),
                            );
                            emit_qwen_ar_cancelled(stdout, &id, 0);
                            batch_clear_terminal(&id, attempt_id);
                            continue;
...
/// Attach batch evidence with EP attestation discipline: a real runtime
/// receipt upgrades the envelope to attested expert_parallel evidence; `None`
/// attaches only the generic continuous-batch route evidence and NEVER
/// fabricates expert_parallel / rank_count / reduce claims (fail closed).
pub fn attach_ep_batch_evidence_fail_closed(
    envelope: &mut serde_json::Value,
    receipt: Option<&qwen35::Qwen35EpBatchReceipt>,
    slots: usize,
    lane: usize,
    lane_capacity: usize,
    max_active_lanes: usize,
    ...
    match receipt {
        Some(receipt) => attach_qwen_ep_batch_receipt_evidence(
            envelope,
            receipt,
            slots,
            lane,
        ...
        None => attach_continuous_batch_route_evidence(
            envelope,
            slots,
            lane,
            lane_capacity,
...
pub fn drive_qwen35_ep_continuous_batch(
    ...
    let fail_all = |sched: &mut ContinuousBatchScheduler,
     ...
     -> Result<(), BatchDriveError> {
        ...
        // Delegate the reset/invalidate/sync chain to the loader-owned helper
        // so this fail_all and `LoadedModel::reset_context` share exactly one
        // implementation (no duplicated reset chain). Ok = every rank reset +
        // graph/replay invalidated + bound + synchronized (attested); Err =
        // aggregated exact failure (unattested).
        let ep = match hipfire_loader::reset_qwen35_ep_batch(batch_state, gpus) {
        ...
        let reason2 = if let Some(ctx) = ep.context.as_deref() {
            format!("{reason}; {ctx}")
        } else {
            reason.clone()
        };
        ...
        Err(BatchDriveError::Gpu(reason2))
    };
    loop {
            ...
            if prompt_tokens.is_empty() || prompt_tokens.len() >= sched.lane_capacity {
                ...
                if let Err(e) = batch_state.reset_lane(gpus, config, lane_idx) {
                    return fail_all(
                        ...
                        batch_state,
                        stdout,
                        format!("EP reset lane {lane_idx} on capacity re-check: {e}"),
                    );
                }
                let _scope = BatchAttemptScope::enter(key.attempt_id);
                emit_uncorrelated_error(
        ...
        for idx in running.clone() {
            ...
            if should_finish {
                ...
                pending_done["latency_ms"] =
                    serde_json::json!((metrics.latency_ms * 10.0).round() / 10.0);
                // EP evidence is fail-closed: attested expert_parallel fields
                // come only from a real runtime receipt; without one, attach
                // only the generic continuous-batch route evidence and never
                // fabricate parallelism/rank_count/reduce claims.
                attach_ep_batch_evidence_fail_closed(
                    &mut pending_done,
                    last_receipt.as_ref(),
                    batch_size,
                    idx,
