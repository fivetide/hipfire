...
/// Pure heterogeneous device-selector preflight: resolves both selectors
/// against visible HIP devices and checks role uniqueness — the single
/// authority shared by the load path and daemon-side preflight. Refuses
/// BEFORE any prior-model teardown. (Cryptographic artifact verification
/// stays inside the load path — it is load-time integrity work, not a
/// request/source-config check.)
pub fn preflight_heterogeneous_devices(
    placement: &hipfire_config::Deepseek4ComputePlacement,
) -> Result<(), String> {
