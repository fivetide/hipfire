...
pub fn resolve_qwen4(raw: &str, head_dim: usize) -> Result<ResolveResult, String> {
    if !raw.is_empty() && normalize_qwen4(raw).is_none() {
        return Err(format!(
            "unsupported qwen4 kv mode {raw:?}; explicit mode must be one of: bf16"
        ));
    }
    Ok(resolve(raw, &QWEN4_POLICY, head_dim))
}
...
mod tests {
    ...
    fn qwen4_is_bf16_only_and_rejects_before_fallback() {
        ...
        for raw in ["q8", "asym3", "garbage"] {
            ...
            assert_eq!(resolve(raw, p, 256).mode, KvMode::Bf16);
            assert!(
                resolve(raw, p, 256).warning.is_some(),
