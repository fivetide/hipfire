...
use crate::ple_rows::{PlePrefetch, PleRowLease, PleRows, PleRowsError, PLE_ROW_BYTES, PLE_ROWS_PER_TOKEN};
use crate::state::{GdnGpuState, QsaGpuState};
use crate::weights::{HyperConnectionWeights, Qwen4LayerWeights, Qwen4Weights, TensorRef, WeightError};
use hipfire_dispatch::context::DispatchCtx;
use hipfire_dispatch::families::gemv::WeightRef;
use hipfire_dispatch::families::moe::{MoeDtypes, MoeParams, RoutedExpertWeights};
use hipfire_dispatch::pipeline::{execute_steps, seal_decode, BoundMoeExperts, ExpertBindingCache, ExpertMetadata, ExpertResource, ExpertResources, ExpertTable, Step};
use hipfire_dispatch::types::dtype_rotation_plan;
use hipfire_runtime::weight_manifest::ExpertSourceLayout;
use rdna_compute::qwen4::{
    qwen4_argmax, qwen4_gdn_conv, qwen4_gdn_gate, qwen4_gdn_params, qwen4_gdn_step,
    qwen4_hc_norm, qwen4_hc_read, qwen4_hc_write, qwen4_qsa_attention,
    qwen4_qsa_cache_append, qwen4_qsa_norm_rope, qwen4_qsa_pool_rope, qwen4_qsa_select,
    qwen4_scale, Qwen4Argmax, Qwen4GdnConv, Qwen4GdnGate, Qwen4GdnParams, Qwen4GdnStep,
...
fn dtype_error(dtype: DType, name: &str) -> Qwen4GpuForwardError {
    invalid(format!("{name} has unsupported resident dtype {dtype:?}"))
}

fn row_stride(dtype: DType, k: usize) -> usize {
    match dtype {
...
fn checked_bytes(rows: usize, stride: usize) -> Result<usize, Qwen4GpuForwardError> {
    rows.checked_mul(stride).ok_or_else(|| invalid("packed row byte size overflow"))
}
...
impl ProjectionView {
    fn from_source(source: &GpuTensor, m: usize, k: usize) -> Self {
        Self {
    ...
    fn from_packed(source: &GpuTensor, offset: usize, bytes: usize, dtype: DType, m: usize, k: usize) -> Self {
        Self {
    ...
    fn dispatch_ref(&self) -> WeightRef<'_> {
        WeightRef {
            buf: &self.tensor,
            dtype: self.dtype,
            m: self.m,
...
impl RoutedExpertWeights for ExpertViewSet<'_> {
    ...
    fn get(&self, expert_idx: usize) -> Option<(WeightRef<'_>, WeightRef<'_>)> {
        self.experts.get(expert_idx).map(|expert| {
            (expert.gate_up.dispatch_ref(), expert.down.dispatch_ref())
        })
    }
}
...
impl Qwen4MoeLayerRuntime {
    fn new(
        ...
        let router = ProjectionView::from_source(router_source, config.num_experts, config.hidden_size);
        ...
        let (gate_up_source_name, down_source_name) = match &moe.expert_source_layout {
            ExpertSourceLayout::PackedFused { gate_up, down, .. } => (gate_up.as_str(), down.as_str()),
            layout => {
                return Err(invalid(format!(
        ...
        for expert in 0..config.num_experts {
            ...
            .map_err(|error| Qwen4GpuForwardError::Dispatch(format!("expert resource: {error:?}")))?;
            ...
            .map_err(|error| Qwen4GpuForwardError::Dispatch(format!("expert resource: {error:?}")))?;
            let resources = ExpertResources::fused(gate_up_resource, down_resource)
                .map_err(|error| Qwen4GpuForwardError::Dispatch(format!("expert resources: {error:?}")))?;
            metadata.push(
                ExpertMetadata::new(expert, 0, expert, resources)
                    .map_err(|error| Qwen4GpuForwardError::Dispatch(format!("expert metadata: {error:?}")))?,
            );
        }
        ...
        let mut cache = table
            ...
            .map_err(|error| Qwen4GpuForwardError::Dispatch(format!("expert binding: {error:?}")))?;
        ...
        if let Err(error) = cache.bind_live(
            ...
            let _ = gpu.free_tensor(expert_gate_up_ptrs);
            let _ = gpu.free_tensor(expert_down_ptrs);
            return Err(Qwen4GpuForwardError::Dispatch(format!("expert live binding: {error:?}")));
        }
        Ok(Self {
            table,
            cache,
...
impl Qwen4GpuForwardScratch {
    pub fn new(gpu: &mut Gpu, config: &Qwen4Config, max_chunk: usize) -> Result<Self, Qwen4GpuForwardError> {
        if max_chunk == 0 {
        ...
        let qsa_index = (config.indexer_n_heads + config.indexer_kv_heads) * config.indexer_head_dim;
        ...
        let max_logits = max_chunk.checked_mul(config.vocab_size).ok_or_else(|| invalid("logit scratch overflow"))?;
        ...
        let result = (|| {
            ...
            alloc(&[max_chunk * PLE_ROWS_PER_TOKEN * (PLE_ROW_BYTES / 2)], DType::BF16)?;
            alloc(&[max_chunk * hidden], DType::F32)?;
...
impl Qwen4GpuForward {
    pub fn new(
        ...
        let result = (|| {
            for layer in &bundle.weights.layer_refs {
                moe.push(Qwen4MoeLayerRuntime::new(gpu, &bundle.weights, layer, &bundle.config)?);
            }
            Ok::<(), Qwen4GpuForwardError>(())
        })();
        if let Err(error) = result {
        ...
        Ok(Self { scratch, moe, ple_epoch: 0 })
    }
    ...
    pub fn forward_token(
        ...
        self.forward_chunk(bundle, gpu, std::slice::from_ref(&token), logits, top1)
    }
    ...
    pub fn forward_chunk(
        ...
        if n == 0 || n > self.scratch.max_chunk {
            return Err(invalid(format!("chunk length {n} exceeds configured capacity {}", self.scratch.max_chunk)));
        }
        let expected_logits = n.checked_mul(bundle.config.vocab_size).ok_or_else(|| invalid("logit shape overflow"))?;
        if logits.dtype != DType::F32 || logits.numel() != expected_logits {
            return Err(invalid(format!("logits must be F32 with {expected_logits} elements")));
        }
        ...
        let attempt = (|| -> Result<(), Qwen4GpuForwardError> {
            ...
            for (token_index, _) in tokens.iter().enumerate() {
                ...
                let embedding_row = f32_view(&embeddings, token_index * config.hidden_size, config.hidden_size);
                ...
                for layer_index in 0..config.num_hidden_layers {
                    if layer_index == config.ple_layer_ids.first().and_then(|id| id.checked_sub(1)).unwrap_or(usize::MAX) {
                        ...
                        self.apply_ple(gpu, &config, &bundle.weights, &mut bundle.state.ple_conv, &ple_rows, token_index)?;
                    }
                    let layer = &bundle.weights.layer_refs[layer_index];
                    self.hc_read(gpu, &config, &bundle.weights, &layer.attn_hyper, streams, &self.scratch.hc_normalized, &self.scratch.hc_low, &self.scratch.hc_up, &self.scratch.hc_mixed)?;
                    let attn_input = &self.scratch.hc_mixed;
                    let attn_output = match layer.kind {
                        LayerType::LinearAttention => {
                            let state = bundle.state.gdn_mut(gdn_slot).ok_or_else(|| invalid("GDN state slot missing"))?;
                            let output = &self.scratch.gdn_output;
                            self.apply_gdn(gpu, &config, &bundle.weights, layer, state, next_position, attn_input, output)?;
                            gdn_slot += 1;
                            output
                        }
                        LayerType::FullAttention => {
                            let state = bundle.state.qsa_mut(qsa_slot).ok_or_else(|| invalid("QSA state slot missing"))?;
                            let output = &self.scratch.qsa_output;
                            self.apply_qsa(gpu, &config, &bundle.weights, layer, state, attn_input, output)?;
                            qsa_slot += 1;
                            output
                        }
                    };
                    self.hc_write(gpu, &config, &bundle.weights, &layer.attn_hyper, streams, &self.scratch.hc_normalized, attn_output, streams)?;
                    self.hc_read(gpu, &config, &bundle.weights, &layer.mlp_hyper, streams, &self.scratch.hc_normalized, &self.scratch.hc_low, &self.scratch.hc_up, &self.scratch.hc_mixed)?;
                    self.apply_moe(gpu, &config, layer_index, &self.moe[layer_index], &self.scratch.hc_mixed, &self.scratch.moe_output)?;
                    self.hc_write(gpu, &config, &bundle.weights, &layer.mlp_hyper, streams, &self.scratch.hc_normalized, &self.scratch.moe_output, streams)?;
                }
                let final_hyper = &bundle.weights.root.final_hyper;
                self.hc_read(gpu, &config, &bundle.weights, final_hyper, &self.scratch.streams, &self.scratch.hc_normalized, &self.scratch.hc_low, &self.scratch.hc_up, &self.scratch.hc_mixed)?;
                let final_hidden = &self.scratch.hc_mixed;
                let lm_head = bundle.weights.resident(&bundle.weights.root.lm_head)?;
                let output_row = f32_view(logits, token_index * config.vocab_size, config.vocab_size);
                self.gemv(gpu, lm_head, final_hidden, &self.scratch.rotation, &output_row, config.vocab_size, config.hidden_size)?;
    ...
    fn gemv(
        ...
        match weight.dtype {
            ...
            dtype => return Err(dtype_error(dtype, "Qwen4 projection")),
        }
        Ok(())
    }
    ...
    fn hc_read(
        ...
        qwen4_hc_norm(gpu, &Qwen4HcNorm { input, norm_weight: norm, normalized, branches: config.hc_count, hidden: config.hidden_size })?;
        self.gemv(gpu, down, normalized, &self.scratch.rotation, low, config.hc_lowrank, config.hc_count * config.hidden_size)?;
        qwen4_scale(gpu, &Qwen4Scale { values: low, scale: 1.0 / config.hc_count as f32 })?;
        gpu.silu_f32(low, low)?;
        self.gemv(gpu, up_weight, low, &self.scratch.rotation, up, config.hc_count * config.hidden_size, config.hc_lowrank)?;
        qwen4_hc_read(gpu, &Qwen4HcRead { input, norm_weight: norm, low, up, normalized, mixed, branches: config.hc_count, hidden: config.hidden_size, rank: config.hc_lowrank })?;
        Ok(())
    }
    ...
    fn hc_write(
        ...
        self.gemv(gpu, inject, normalized, &self.scratch.rotation, gates, config.hc_count, config.hc_count * config.hidden_size)?;
        qwen4_hc_write(gpu, &Qwen4HcWrite { input, normalized, mixed, gates, output, branches: config.hc_count, hidden: config.hidden_size })?;
        Ok(())
    }
    ...
    fn apply_gdn(
        ...
        let gdn = layer.gdn.as_ref().ok_or_else(|| invalid("linear layer has no GDN weights"))?;
        ...
        self.gemv(gpu, qkv_weight, input, &self.scratch.rotation, &self.scratch.projection, qkv, config.hidden_size)?;
        let history_rows = config.linear_conv_kernel_dim.saturating_sub(1);
        let cursor = if history_rows == 0 { 0 } else { position % history_rows };
        qwen4_gdn_conv(gpu, &Qwen4GdnConv { input: &self.scratch.projection, kernel: conv, history: &state.conv, output: &self.scratch.projection2, next_history: &state.conv, channels: qkv, history_rows, kernel_size: config.linear_conv_kernel_dim, cursor })?;
        let a_weight = weights.resident(&gdn.in_proj_a)?;
        let b_weight = weights.resident(&gdn.in_proj_b)?;
        self.gemv(gpu, a_weight, input, &self.scratch.rotation, &self.scratch.gdn_a, config.linear_num_value_heads, config.hidden_size)?;
        self.gemv(gpu, b_weight, input, &self.scratch.rotation, &self.scratch.gdn_b, config.linear_num_value_heads, config.hidden_size)?;
        let a_log = weights.resident(&gdn.a_log)?;
        let dt_bias = weights.resident(&gdn.dt_bias)?;
        qwen4_gdn_params(gpu, &Qwen4GdnParams { a: &self.scratch.gdn_a, b: &self.scratch.gdn_b, a_log, dt_bias, gate: &self.scratch.gdn_gate, beta: &self.scratch.gdn_beta }, config.linear_num_value_heads)?;
        ...
        qwen4_gdn_step(gpu, &Qwen4GdnStep { q: &q, k: &k, v: &v, gate: &self.scratch.gdn_gate, beta: &self.scratch.gdn_beta, state: &state.recurrent, output: &self.scratch.gdn_recurrent_output, key_heads: config.linear_num_key_heads, value_heads: config.linear_num_value_heads, key_dim: config.linear_key_head_dim, value_dim: config.linear_value_head_dim })?;
        let z_weight = weights.resident(&gdn.z)?;
        self.gemv(gpu, z_weight, input, &self.scratch.rotation, &self.scratch.gdn_z, value, config.hidden_size)?;
        let norm = weights.resident(&gdn.norm)?;
        qwen4_gdn_gate(gpu, &Qwen4GdnGate { recurrent_output: &self.scratch.gdn_recurrent_output, z: &self.scratch.gdn_z, norm, output: &self.scratch.gdn_output, value_heads: config.linear_num_value_heads, value_dim: config.linear_value_head_dim })?;
        let out_weight = weights.resident(&gdn.output)?;
        self.gemv(gpu, out_weight, &self.scratch.gdn_output, &self.scratch.rotation, output, config.hidden_size, value)?;
        Ok(())
    }
    ...
    fn apply_qsa(
        ...
        let qsa = layer.attention.as_ref().ok_or_else(|| invalid("full layer has no QSA weights"))?;
        ...
        self.gemv(gpu, index_weight, input, &self.scratch.rotation, &self.scratch.qsa_index, index_width, config.hidden_size)?;
        let index_q = view(&self.scratch.qsa_index, 0, index_q_width);
        let index_k = view(&self.scratch.qsa_index, index_q_width, config.indexer_kv_heads * index_dim);
        let index_q_norm = weights.resident(&qsa.indexer_q_norm)?;
        let index_k_norm = weights.resident(&qsa.indexer_k_norm)?;
        qwen4_qsa_norm_rope(gpu, &Qwen4QsaNormRope { values: &index_q, norm: index_q_norm, heads: config.indexer_n_heads, head_dim: index_dim, position: state.position, rotary_dim: index_dim.min(64) })?;
        qwen4_qsa_norm_rope(gpu, &Qwen4QsaNormRope { values: &index_k, norm: index_k_norm, heads: config.indexer_kv_heads, head_dim: index_dim, position: 0, rotary_dim: index_dim.min(64) })?;
        gpu.memcpy_dtod_at_auto(&state.raw_index_keys.buf, state.position * config.indexer_kv_heads * index_dim * 4, &index_k.buf, 0, config.indexer_kv_heads * index_dim * 4)?;
        ...
        self.gemv(gpu, q_weight, input, &self.scratch.rotation, &self.scratch.qsa_qgate, 2 * q_width, config.hidden_size)?;
        self.gemv(gpu, k_weight, input, &self.scratch.rotation, &self.scratch.qsa_k, kv_width, config.hidden_size)?;
        self.gemv(gpu, v_weight, input, &self.scratch.rotation, &self.scratch.qsa_v, kv_width, config.hidden_size)?;
        ...
        qwen4_qsa_norm_rope(gpu, &Qwen4QsaNormRope { values: &q_values, norm: q_norm, heads: config.num_attention_heads, head_dim: config.head_dim, position: state.position, rotary_dim: config.head_dim.min(64) })?;
        qwen4_qsa_norm_rope(gpu, &Qwen4QsaNormRope { values: &self.scratch.qsa_k, norm: k_norm, heads: config.num_key_value_heads, head_dim: config.head_dim, position: state.position, rotary_dim: config.head_dim.min(64) })?;
        qwen4_qsa_cache_append(gpu, &Qwen4QsaCacheAppend { key: &self.scratch.qsa_k, value: &self.scratch.qsa_v, full_keys: &state.full_keys, full_values: &state.full_values, position: state.position, kv_width })?;
        let visible = state.position + 1;
        let complete = visible / config.indexer_compress_ratio;
        if complete > 0 {
            qwen4_qsa_pool_rope(gpu, &Qwen4QsaPoolRope { raw_keys: &state.raw_index_keys, pooled: &state.pooled_keys, block_count: complete, compress: config.indexer_compress_ratio, index_dim: config.indexer_kv_heads * index_dim })?;
        }
        let budget_blocks = config.indexer_budget / config.indexer_compress_ratio;
        qwen4_qsa_select(gpu, &Qwen4QsaSelect { query: &index_q, pooled: &state.pooled_keys, selected: &state.selected_indices, block_count: complete, index_heads: config.indexer_n_heads, index_dim, budget_blocks, compress: config.indexer_compress_ratio, visible, capacity: state.selected_capacity })?;
        let selected = budget_blocks.min(complete) * config.indexer_compress_ratio + visible - complete * config.indexer_compress_ratio;
        let selected = selected.min(state.selected_capacity);
        qwen4_qsa_attention(gpu, &Qwen4QsaAttention { q_with_gate: &self.scratch.qsa_qgate, full_keys: &state.full_keys, full_values: &state.full_values, selected: &state.selected_indices, output: &self.scratch.qsa_output, n_heads: config.num_attention_heads, n_kv_heads: config.num_key_value_heads, head_dim: config.head_dim, selected_len: selected, full_capacity: state.full_capacity })?;
        let out_weight = weights.resident(&qsa.output)?;
        self.gemv(gpu, out_weight, &self.scratch.qsa_output, &self.scratch.rotation, output, config.hidden_size, q_width)?;
    ...
    fn apply_ple(
        ...
        let row = f32_view(ple_rows, token_index * config.hidden_size, config.hidden_size);
        let key_weight = weights.resident(&ple.key)?;
        let value_weight = weights.resident(&ple.value)?;
        self.gemv(gpu, key_weight, &row, &self.scratch.rotation, &self.scratch.ple_key, channels, config.hidden_size)?;
        self.gemv(gpu, value_weight, &row, &self.scratch.rotation, &self.scratch.ple_value, config.hidden_size, config.hidden_size)?;
        gpu.copy_d2d(&self.scratch.streams, &self.scratch.ple_query, self.scratch.ple_query.byte_size())?;
        let norm_key = weights.resident(&ple.norm_key)?;
        let norm_query = weights.resident(&ple.norm_query)?;
        gpu.qwen4_ple_gate_bf16(&self.scratch.ple_key, &self.scratch.ple_query, &self.scratch.ple_value, norm_key, norm_query, &self.scratch.ple_gated, 1, config.hc_count, config.hidden_size, EPSILON)?;
        let norm_conv = weights.resident(&ple.norm_conv)?;
        gpu.qwen4_ple_norm_bf16(&self.scratch.ple_gated, norm_conv, &self.scratch.ple_normed, 1, config.hc_count, config.hidden_size, EPSILON)?;
        let conv = weights.resident(&ple.conv)?;
        gpu.qwen4_ple_depthwise_conv_silu_add_bf16(&self.scratch.ple_gated, &self.scratch.ple_normed, conv, state, &self.scratch.ple_output, 1, channels, config.ple_conv_kernel_size, 3)?;
        gpu.add_f32(&self.scratch.streams, &self.scratch.ple_output, &self.scratch.streams)?;
        Ok(())
    }
    ...
    fn apply_moe(
        ...
        let dtypes = MoeDtypes {
            ...
            experts_all_gate_up_mq4: runtime.experts.iter().all(|expert| expert.gate_up.dtype == DType::MQ4G256V2),
            routed_gate_up: runtime.experts.first().map(|expert| expert.gate_up.dtype).ok_or_else(|| invalid("MoE expert table is empty"))?,
            routed_down: runtime.experts.first().map(|expert| expert.down.dtype).ok_or_else(|| invalid("MoE expert table is empty"))?,
