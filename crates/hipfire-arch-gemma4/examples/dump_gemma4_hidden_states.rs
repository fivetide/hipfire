...
//! Dump per-layer hidden states from the Gemma 4 dense forward for offline
...
//!
//! Usage:
//!   dump_gemma4_hidden_states --model <hfq> --ref <kldref> --out <hfhs> [--chunk N]
...
fn main() {
    ...
    use hipfire_arch_gemma4::forward::decode_step_capture;
    ...
    eprintln!(
        "arch=gemma4 dim={} layers={} heads={} sliding(hd={},kv={}) full(hd={},kv={}) vocab={}",
        cfg.dim,
        cfg.n_layers,
        cfg.n_heads,
    ...
    let mut state = Gemma4State::new_with_max_seq(&mut gpu, &cfg, n_ctx + 16).expect("state");

    // ---- per-token forward with per-layer capture ----
    let mut capture: Vec<Vec<f32>> =
        vec![Vec::with_capacity(n_ctx * cfg.dim); cfg.n_layers];
    ...
    out.write_all(&(cfg.n_layers as u32).to_le_bytes()).unwrap();
    out.write_all(&(n_ctx as u32).to_le_bytes()).unwrap();
    out.write_all(&(cfg.dim as u32).to_le_bytes()).unwrap();
    out.write_all(&0u32.to_le_bytes()).unwrap();
    for (l, buf) in capture.iter().enumerate() {
        assert_eq!(buf.len(), n_ctx * cfg.dim, "layer {l} size");
        let bytes: &[u8] =
